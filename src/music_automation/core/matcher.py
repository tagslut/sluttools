    ...
    if not results:
        ...
        if playlist_size <= 50:
            ...
            if not search_terms:
                ...
                # Targeted search: only check files that contain our search terms
                matches_found = 0
                for orig_path, norm_basename in flac_lookup:
                    # Quick pre-filter: only process files likely to match
                    if any(term in norm_basename for term in search_terms):
                        r = combined_fuzzy_ratio_cached(ss_norm, norm_basename)
                        if r >= AUTO_MATCH_THRESHOLD:
                            return orig_path
                        results.append((orig_path, r))
...
async def main():
    ...
    # Set OUTPUT_DIR to the directory containing the input file/folder
    global OUTPUT_DIR
    OUTPUT_DIR = Path(path_in).parent if os.path.isfile(path_in) else Path(path_in)
        ...
        try:
            with open(cache_file, 'r') as f:
                ...
                if len(cached_data) == len(all_flacs):
                    flac_lookup = [(item["path"], item["norm"]) for item in cached_data]
    ...
    if opt_choice in ["3", "4", "5"] and total_entries > 5:
