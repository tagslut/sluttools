...
def create_parser():
    ...
    match_parser = subparsers.add_parser(
        "match", help="Match playlist tracks to FLAC library"
    )
    match_parser.add_argument(
        "playlist", nargs="?", help="Path to playlist file or folder"
    )

    # Copy playlist command (from copy_playlist.py)
    copy_parser = subparsers.add_parser(
        "copy", help="Copy files from a playlist to a destination directory"
    )
    copy_parser.add_argument("playlist", help="Path to M3U playlist file")
    copy_parser.add_argument("destination", help="Destination directory")
    copy_parser.add_argument(
        "-f", "--flat", action="store_true", help="Flat copy (no directory structure)"
    )
    copy_parser.add_argument(
        "-p", "--preserve", action="store_true", help="Preserve directory structure"
    )
    copy_parser.add_argument(
        "-a",
        "--artist-folders",
    ...
    copy_parser.add_argument(
        "-t", "--threads", type=int, default=4, help="Number of parallel copy threads"
    )

    # === FLAC DATABASE COMMANDS ===

    # Database refresh command (from flacdb.py)
    db_parser = subparsers.add_parser("db", help="FLAC database management")
    ...
    refresh_parser.add_argument(
        "--db", default=get_default_db_path(), help="Path to SQLite database"
    )
    refresh_parser.add_argument(
    ...
    # DB List command
    list_parser = db_subparsers.add_parser("list", help="List entries in the database")
    ...
    show_parser = db_subparsers.add_parser(
        "show", help="Show details for a single entry"
    )
    ...
    resample_parser = subparsers.add_parser(
        "resample", help="Batch resample FLAC files"
    )
    ...
    resample_parser.add_argument(
        "--threads", type=int, default=4, help="Number of parallel resampling threads"
    )
...
def show_help():
    ...
    table.add_row(
        "db list", "List entries in the FLAC database", "musictools db list --limit 10"
    )
    table.add_row(
        "db show",
    ...
    table.add_row("resample", "Batch resample FLAC files", "musictools resample")
...
def run_script(script_path, args=None):
    ...
    try:
        ...
        console.print(
            f"[bold red]Error running {os.path.basename(script_path)}: {e}[/bold red]"
        )
        return False
    ...
    try:
        ...
        if args.command == "match":
            script_path = os.path.join(
                os.path.dirname(__file__), "..", "core", "matcher.py"
            )
            if os.path.exists(script_path):
                ...
                console.print(
                    f"[bold red]Error: Playlist matching script not found at {script_path}[/bold red]"
                )

        # Handle playlist copying
        elif args.command == "copy":
            script_path = os.path.join(
                os.path.dirname(__file__), "..", "core", "copier.py"
            )
            if os.path.exists(script_path):
                ...
                console.print(
                    f"[bold red]Error: Playlist copy script not found at {script_path}[/bold red]"
                )
        ...
        # Handle database commands
            ...
            if not os.path.exists(script_path):
                console.print(
                    f"[bold red]Error: FLAC database script not found at {script_path}[/bold red]"
                )
                return
