...
from .matching import find_matches, get_playlist_tracks, write_match_json, write_match_m3u, write_songshift_json, perform_matching_with_review
...
out_app = typer.Typer(help="Export results: create M3U, JSON mapping, or SongShift JSON from a playlist input")
...
async def animate_title(refresh_rate: float = 0.2, wait_time: float = 4.0, plain: bool = False):
...
def get_library():
    ...
    if not config.get('LIBRARY_ROOTS'):
        console.print("[bold red]No library roots configured. Please run 'slut config edit' first.[/bold red]")
        raise typer.Exit(1)
...
def get_playlist(
    ...
    if not tracks:
        ...
        raise typer.Exit(1)
    console.print(f"[green]Loaded {len(tracks)} track(s) from {Path(playlist).name}[/green]")
...
async def _interactive_match_async(playlist: str, plain: bool = False, no_refresh: bool = False):
    ...
    if unmatched_count > 0 and safe_confirm("[bold yellow]Export unmatched tracks as a SongShift-ready JSON playlist?[/bold yellow]"):
    ...
    if matched_paths and safe_confirm("[bold yellow]Create .m3u with the current matched flacs?[/bold yellow]"):
        default_name = f"{playlist_name}_matched.m3u"
        m3u_filename = safe_prompt("M3U filename", default=default_name).strip().strip("'\"")
