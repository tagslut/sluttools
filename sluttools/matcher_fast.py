...
"""
import os, re, csv, sys, json, time, argparse, sqlite3, unicodedata
...
try:
    ...
    def start_progress(desc: str):
        ...
        p.start(); tid = p.add_task(desc, total=None); return p, tid
    ...
    def track_iter(desc: str, total: int):
        ...
        p.start(); tid = p.add_task(desc, total=total)
        def adv(): p.advance(tid)
        def end(): p.stop()
        return adv, end
except Exception:
    console = None
    def info(msg):
        import re as _re
        print(_re.sub(r"\[[^\]]+\]", "", str(msg)))
    def start_progress(desc: str): print(desc + " ..."); return None, None
    def stop_progress(p): ...
    def track_iter(desc: str, total: int):
        c={"i":0}
        def adv():
...
GENERIC_PAT = re.compile(r"\[unknown artist\]|\(xxxx\)\s*\[unknown album\]|/unknown/|/various/", re.IGNORECASE)
...
def open_db(db_path: str) -> sqlite3.Connection:
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS flacs (path TEXT PRIMARY KEY, norm TEXT NOT NULL, mtime INTEGER NOT NULL)""")
...
def meta_get(conn: sqlite3.Connection, key: str, default: Optional[str]=None) -> Optional[str]:
    cur = conn.cursor(); cur.execute("SELECT v FROM meta WHERE k=?", (key,)); row = cur.fetchone()
...
def need_refresh(conn: sqlite3.Connection, stale_seconds: int) -> bool:
    cur = conn.cursor(); cur.execute("SELECT COUNT(*) FROM flacs"); count = cur.fetchone()[0]
                ...
                for e in it:
                    if e.is_dir(follow_symlinks=False): stack.append(e.path)
                        ...
                        if e.name.startswith("._"):
                            continue
                        try: yield e.path, int(e.stat().st_mtime)
...
def refresh_library(library_dir: str, db_path: str) -> int:
    ...
    if to_delete: cur.executemany("DELETE FROM flacs WHERE path=?", to_delete)

    batch_new=[]; batch_upd=[]; commit_every=2000; seen=0
    ...
    try:
        for p_str, mtime in iter_flacs(library_dir):
            ...
            if (len(batch_new)+len(batch_upd))>=commit_every:
                if batch_new:
                    cur.executemany("INSERT OR REPLACE INTO flacs(path,norm,mtime) VALUES (?,?,?)", batch_new); batch_new.clear()
                if batch_upd:
                    cur.executemany("UPDATE flacs SET norm=?, mtime=? WHERE path=?", batch_upd); batch_upd.clear()
                conn.commit()
    finally:
    ...
    meta_set(conn, "last_scan_ts", str(int(time.time()))); conn.commit(); conn.close()
    info(f"[green]Index refreshed. Scanned {seen} candidate files.[/green]")
    return seen

def fetch_lookup(db_path: str) -> List[Tuple[str,str]]:
    conn=open_db(db_path); cur=conn.cursor(); cur.execute("SELECT path, norm FROM flacs"); rows=cur.fetchall(); conn.close(); return rows

# --------- Parsers ---------
def parse_m3u(file_path: str):
    tracks=[]; encodings=("utf-8","iso-8859-1","windows-1252")
    for enc in encodings:
        try:
            ...
            info(f"[red]Error reading M3U: {e}[/red]"); break
    return Path(file_path).stem, tracks
...
def parse_json_playlist(file_path: str):
    try:
        ...
        for r in data.get("tracks",[]):
            out.append({"artist":r.get("artist",""),"album":r.get("album",""),"track":r.get("track",""),"title":r.get("title","")})
        return name, out
    except Exception as e:
...
def parse_csv_playlist(file_path: str):
    ...
    try:
        with open(file_path,"r",encoding="utf-8") as cf:
            ...
            for row in rd:
                out.append({"track":row.get("track") or row.get("title",""),"artist":row.get("artist",""),"album":row.get("album",""),"title":row.get("title","")})
...
def parse_playlist_file(path: str):
    ...
    if ext==".m3u": return parse_m3u(path)
    if ext==".json": return parse_json_playlist(path)
    if ext==".csv": return parse_csv_playlist(path)
    if ext in (".xlsx",".xls"): return parse_xlsx_playlist(path)
    info("[red]Unsupported playlist format.[/red]"); return Path(path).stem, []
        ...
        for pth, norm_lib in lookup:
            ...
            if adj > best_score:
                ...
                if best_score >= 95 and not want_topk:
                    return MatchInfo(best_path, "fuzzy", int(best_score), best_key, None)
    topk = None
...
def write_m3u(out_path: str, matched_paths: List[str]) -> None:
    if not out_path.lower().endswith(".m3u"): out_path += ".m3u"
    with open(out_path,"w",encoding="utf-8") as f:
        f.write("#EXTM3U\n")
        for p in matched_paths: f.write(p+"\n")
...
# --------- CLI ---------
def run_match(args):
    db_path=args.db; lib_dir=args.library; stale=args.stale_seconds
    ...
    if args.refresh=="yes":
        ...
        conn=open_db(db_path); do=need_refresh(conn, stale); conn.close()
        if do:
    ...
    lookup=fetch_lookup(db_path); exact_map=build_exact_map(lookup)
    info(f"[green]FLAC index entries:[/green] {len(lookup)}")

    src=args.input; entries=[]; playlist_name=Path(src).stem
    ...
    explains: List[MatchInfo] = [MatchInfo(None,"none",0,"",None) for _ in range(total)]

    adv,end=track_iter("Matching tracks", total=total)
    for i,e in enumerate(entries,1):
        mi = match_entry(e, exact_map, lookup, threshold,
    ...
    n_matched=sum(1 for m in matched if m); n_unmatched=total-n_matched
    ...
    def print_compact(rows_idx):
        ...
        if Table is None or not console:
            ...
            info(header)
            for i in rows_idx:
                e = entries[i]; mi = explains[i]
                q = ellipsize(as_query(e), args.truncate)
                path_disp = ellipsize(fmt_path(mi.path, args.path_col, args.relative_root), args.truncate)
        ...
        if args.view == "full":
            for i, e in enumerate(entries):
                ...
                if mi.path:
                    ...
                    info(f"[green]{i+1:03d}[/green] {ellipsize(q, args.truncate)}  →  [cyan]{ellipsize(path_disp, args.truncate)}[/cyan]  "
                         f"[dim]({mi.method}, score={mi.score}, key='{ellipsize(mi.key_used, args.truncate)}')[/dim]")
                    if args.topk and mi.topk:
                        tops = ", ".join(f"{s}:{Path(p).name}" for s,p in mi.topk[:args.topk])
                        info(f"      top{args.topk}: {ellipsize(tops, args.truncate)}")
                else:
                    info(f"[red]{i+1:03d}[/red] {ellipsize(q, args.truncate)}  →  [bold]UNMATCHED[/bold]  "
    ...
    if args.m3u:
        ...
        if final_paths: write_m3u(args.m3u, final_paths)
        else: info("[yellow]No matches to write to M3U.[/yellow]")
...
def main():
    ...
    apm=sub.add_parser("match", help="Match a playlist or directory against the FLAC library")
    apm.add_argument("input", help="Path to playlist file (.json/.m3u/.csv/.xlsx) or a directory")
    apm.add_argument("--library", default=DEFAULT_LIBRARY, help=f"FLAC library root (default: {DEFAULT_LIBRARY})")
    apm.add_argument("--db", default=DEFAULT_DB, help=f"SQLite DB path (default: {DEFAULT_DB})")
    apm.add_argument("--refresh", choices=("auto","yes","no"), default="auto", help="Refresh index: auto (only if stale), yes (force), no (skip)")
    apm.add_argument("--stale-seconds", type=int, default=21600, help="Index staleness window (default: 6h)")
    apm.add_argument("--m3u", default=None, help="Write matched result to this .m3u")
    apm.add_argument("--export-unmatched", default=None, help="Write unmatched as SongShift JSON to this path")
    apm.add_argument("--service", default="qobuz", help="Service label for exported JSON (default: qobuz)")
    apm.add_argument("--threshold", type=int, default=None, help="Fuzzy accept threshold (overrides env)")
    # views & transparency defaults
    apm.add_argument("--view", choices=("compact","unmatched","full"), default="compact",
                     help="How to render results: compact table (default), only unmatched, or full verbose")
    apm.add_argument("--no-explain", dest="explain", action="store_false",
                     help="Disable per-track output")
    apm.add_argument("--topk", type=int, default=5, help="When --view full, show top-K candidates (0=off)")
    apm.add_argument("--truncate", type=int, default=80, help="Max column width; 0 = no truncation")
    apm.add_argument("--path-col", choices=("basename","relative","full"), default="basename",
                     help="How to display matched path")
    apm.add_argument("--relative-root", default=None,
                     help="Base directory used when --path-col=relative")
    apm.add_argument("--no-color", action="store_true", help="Disable colored output")
    apm.add_argument("--print-paths", action="store_true", help="Print only matched paths to stdout")
    # control substring fast path
    apm.add_argument("--no-substr", dest="allow_substr", action="store_false", help="Disable guarded substring/token-overlap fast path")
...
if __name__=="__main__":
    try: main()
        ...
        info("[red]Interrupted by user. Exiting.[/red]"); sys.exit(130)
