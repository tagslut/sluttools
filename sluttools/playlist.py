...
def update_library_from_playlist(
    tracks: list[dict], database: DatabaseInterface = None
) -> dict:
    """
    ...
    try:
        for i, track in enumerate(tracks):
            ...
            if not track_id:
                logger.warning(
                    "Track at index %d has no valid ID (missing 'id' or 'track_id'). Skipping.",
                    i,
                )
            ...
            if database.track_exists(track_id):
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        "Track (%s) already exists in database. Skipping.", track_id
                    )
        ...
        if skipped_invalid_count > 0:
            logger.warning(
                "Skipped %d tracks due to missing or invalid IDs", skipped_invalid_count
            )

        logger.info(
            "Library update complete: %d new/updated, %d already indexed, %d invalid",
