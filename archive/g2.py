...
def combined_fuzzy_ratio_cached(a: str, b: str) -> int:
    """Cached version of combined_fuzzy_ratio for better performance"""
    return combined_fuzzy_ratio(a, b)
...
def match_entry(
    ...
    # If no match found with indexes or indexes not available, try full scan with early termination
    if not results:
        ...
        for orig_path, norm_basename in flac_lookup:
            ...
            # Early termination: if we find a very good match, return immediately
            ...
            # Optimization: Only check the first N entries if the list is long
            if len(results) >= 500 and len(flac_lookup) > 1000:
                break
...
async def main():
    ...
    console.print("[bold yellow]Choose optimization level:[/bold yellow]")
    ...
    opt_choice = safe_prompt("Optimization level [1-4]", default="3")

    # Pre-compute normalized basenames for faster matching
    ...
    if opt_choice in ["2", "3"]:
        try:
            artist_index, title_index = build_search_indexes(all_flacs)
        except Exception as e:
    ...
    # Use parallel processing for Turbo modes
    if opt_choice in ["3", "4"] and total_entries > 5:
        ...
        def match_worker(idx_entry):
            ...
            result = match_entry(entry, flac_lookup, artist_index, title_index)
        ...
        for i, e in enumerate(entries):
            ...
            res = match_entry(e, flac_lookup, artist_index, title_index)
    ...
    if auto_unmatched_count > 0 and safe_confirm("[bold yellow]Do you want to attempt manual matching on unmatched tracks?[/bold yellow]"):
        ...
        for idx in unmatched_indices:
            ...
            if res == "QUIT":
                console.print("[yellow]User opted to quit manual matching.[/bold yellow]")
                break
    ...
    if final_matches and safe_confirm("[bold yellow]Create CSV with the current matched flacs?[/bold yellow]"):
        ...
        console.print("[yellow]Skipping CSV creation.[/bold yellow]")
...
def build_search_indexes(flac_files: List[str]) -> Tuple[Dict[str, Set[str]], Dict[str, Set[str]]]:
    ...
    console.print("[cyan]Building search indexes for faster matching...[/cyan]")
    with console.status("[green]Indexing...", spinner="dots12"):
        ...
        for i in range(0, total, chunk_size):
            ...
            for f in chunk:
                try:
                    # Extract file metadata with a timeout
                    meta = {}
                    try:
...
if __name__ == "__main__":
    try:
        ...
        console.print("[red]Process interrupted by user (Ctrl+C). Exiting...[/bold red]")
        sys.exit(0)
