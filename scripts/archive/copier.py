#!/usr/bin/env python3
# Script: Copies all files referenced by an M3U playlist into a destination folder.
"""
Copy Playlist Files
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Copy music files from a playlist to a destination directory"
    )
...
def main():
    ...
    if not os.path.isfile(args.playlist):
        console.print(
            f"[bold red]Error: Playlist file not found: {args.playlist}[/bold red]"
        )
        sys.exit(1)
        ...
        if args.flat:
            ...
            if len(parts) >= 3:
                ...
                rel_path = os.path.relpath(
                    source, os.path.dirname(os.path.dirname(source))
                )
...
def create_schema(db_path):
    ...
    conn.commit()
    conn.close()
    print(f"Database schema created or verified at {db_path}")


if len(sys.argv) >= 3 and sys.argv[1] == "init-db":
    create_schema(sys.argv[2])
    sys.exit(0)
